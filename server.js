...
// ── HUMN IQ — AI Card Grading endpoint ───────────────────────────────────────
  ...
  try {
    const response = await fetch("https://api.anthropic.com/v1/messages", {
      ...
      body: JSON.stringify({
        ...
        max_tokens: 1000,
        system: "You are an expert Pokemon TCG card grader with years of experience grading for PSA, BGS, and CGC. Analyze card images and provide detailed grading assessments. Always respond with valid JSON only, no markdown, no explanation outside the JSON.",
        messages: [{
          role: "user",
          content: [
            {
            ...
            {
              type: "text",
              text: 'Grade this Pokemon card like a professional PSA grader. Respond ONLY with this exact JSON format: {"cardName":"card name if visible","set":"set name if visible","overallGrade":8.5,"psaEquivalent":"PSA 8","centering":{"score":9,"frontLeftRight":"55/45","frontTopBottom":"52/48","notes":"Slightly off-center left to right"},"corners":{"score":8,"notes":"Minor wear on top-right corner, others clean"},"edges":{"score":9,"notes":"Clean edges with minimal whitening"},"surface":{"score":8,"notes":"Light scratch near center, print is clean"},"submitRecommendation":true,"submitReason":"Strong candidate for PSA 8-9. Surface scratch may cost a point.","estimatedValue":{"raw":"$45-60","psa8":"$120-150","psa9":"$200-250","psa10":"$500+"}}'
            }
          ]
        }]
      })
    ...
    log.error("Grade API error", { error: err.message });
